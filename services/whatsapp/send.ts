...
export async function sendWhatsAppText(to: string, body: string) {
  ...
  const phoneId = await getSetting<string>('whatsapp.phone_number_id');
  const enabled = await getSetting<boolean>('whatsapp.enabled');
  if (!enabled) return { sent: false, dryRun: true, to, body };
  ...
  return { sent: true, dryRun: false, response: json };
}
