...
const SYSTEM_USER: SafeUser = {
  id: 'DONI_AUTOMATION',
  username: 'doni-automation',
  fullName: 'DONI Automation',
  email: null,
  role: 'SUPER_ADMIN',
  country: null,
  active: true,
...
function dueToken(value: string) {
...
async function escalationUsers() {
  ...
  for (const row of rows) {
    const [department, orgRole] = await Promise.all([
      getUserDepartment(row.id),
      getUserOrgRole(row.id, row.role),
    ]);
...
export async function runCaseReminderAutomation() {
  ...
  for (const c of rows) {
    ...
    for (const manager of managers) {
      if (await case360Accessible(c.kind as any, c.caseId, manager).catch(() => false))
        eligibleManagers.push(manager);
    }
  ...
  return {
    ...
    managerEscalationMinutes: MANAGER_ESCALATION_MINUTES,
  };
}
