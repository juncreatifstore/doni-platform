...
import {
  allowedTicketReferences,
  conversationCountryWhere,
  countryWhere,
  dataScopeForUser,
...
export async function getReservationsWorkCenter(scope: DataScope = { mode: 'global' }) {
  ...
  const ticketWhere = {
    status: { in: ['CREATED', 'TO_BE_PAID', 'PENDING_MANUAL_ISSUE', 'ISSUING'] },
    ...conversationCountryWhere(scope),
  };
  ...
  const [pendingPayments, ticketsToIssue, activeConversations, recentTickets] = await Promise.all([
    prisma.payment.findMany({
      ...
      take: 8,
      select: { reference: true, amount: true, currency: true, provider: true, updatedAt: true },
    }),
    prisma.ticket.findMany({
      where: ticketWhere,
      orderBy: { updatedAt: 'asc' },
      take: 8,
      select: { reference: true, status: true, pnr: true, updatedAt: true },
    }),
    prisma.doniConversation.findMany({
    ...
    prisma.ticket.findMany({
      ...
      take: 6,
      select: { reference: true, pnr: true, deliveryStatus: true, issuedAt: true },
    }),
  ]);
...
export async function getCustomerServiceWorkCenter(scope: DataScope = { mode: 'global' }) {
  ...
  const [agentRequired, rawPostBooking, rawCheckins, rawBaggage] = await Promise.all([
    prisma.doniConversation.findMany({
      where: { status: 'ACTIVE', agentRequired: true, ...countryWhere(scope) },
      orderBy: { updatedAt: 'asc' },
      take: 10,
      select: { id: true, waId: true, country: true, language: true, currentSegment: true, updatedAt: true },
    }),
    scope.mode === 'none'
      ? Promise.resolve([])
    ...
    scope.mode === 'none'
      ? Promise.resolve([])
      : prisma.checkinService.findMany({
    ...
    scope.mode === 'none'
      ? Promise.resolve([])
      : prisma.baggageService.findMany({
          where: { status: { notIn: ['completed', 'cancelled'] } },
          orderBy: { updatedAt: 'asc' },
          take: 30,
          select: {
            ...
            weightKg: true,
            paymentStatus: true,
            tracking: { select: { ticketReference: true, clientPhone: true } },
          },
        }),
  ]);
  ...
  return {
    agentRequired,
    postBooking: rawPostBooking.filter((x: any) => inScope(x.reference)).slice(0, 10),
    checkins: rawCheckins.filter((x: any) => inScope(x.tracking?.ticketReference)).slice(0, 8),
    baggage: rawBaggage.filter((x: any) => inScope(x.tracking?.ticketReference)).slice(0, 8),
  };
}
...
export async function getFinanceWorkCenter(scope: DataScope = { mode: 'global' }) {
  ...
  const [manualReviews, pendingPayments, rawRefunds, failedPayments] = await Promise.all([
    scope.mode === 'none'
      ...
      : prisma.manualPaymentReview.findMany({
          where: { status: { in: ['PENDING', 'NEEDS_INFO'] }, payment: paymentScope },
    ...
    prisma.payment.findMany({
      where: { status: { in: ['FAILED', 'EXPIRED'] }, ...paymentScope },
  ...
  if (scope.mode === 'country' && rawRefunds.length) {
    const refs = rawRefunds.map((x: any) => x.ticketReference).filter(Boolean);
    const allowed = refs.length
      ? await prisma.ticket.findMany({
          where: { reference: { in: refs }, ...conversationCountryWhere(scope) },
          select: { reference: true },
        })
      : [];
    const allowedRefs = new Set(allowed.map((x: any) => x.reference));
    refunds = rawRefunds.filter((x: any) => x.ticketReference && allowedRefs.has(x.ticketReference));
  }
...
export async function getMarketingWorkCenter(inputScope?: DataScope) {
  ...
  const customerScope = countryWhere(scope),
    conversationScope = countryWhere(scope),
    paymentScope = conversationCountryWhere(scope);
  const [newCustomers, started, paid, topCountries, recentCustomers] = await Promise.all([
    ...
    prisma.customerProfile.groupBy({
      by: ['country'],
      where: { createdAt: { gte: week }, ...customerScope },
      _count: { _all: true },
  ...
  return {
    ...
    paid,
    conversion,
    topCountries: (topCountries as any[]).map((x: any) => ({
      country: x.country || 'Non défini',
...
export async function getManagementWorkCenter(inputScope?: DataScope) {
  const scope = await resolveScope(inputScope);
  const refs = await allowedTicketReferences(scope);
  const refundRefWhere = refs === null ? {} : { ticketReference: { in: refs } };
  const [users, activeConversations, pendingRefunds, pendingReviews, ticketsToIssue] = await Promise.all([
    ...
    prisma.refundRequest.count({
      where: { status: { notIn: ['completed', 'rejected', 'cancelled'] }, ...refundRefWhere },
    }),
    prisma.manualPaymentReview.count({
      where: { status: { in: ['PENDING', 'NEEDS_INFO'] }, payment: conversationCountryWhere(scope) },
    }),
    prisma.ticket.count({
  ...
  const openIncidents =
    scope.mode === 'global'
      ? await prisma.flightIncident.count({ where: { status: { in: ['open', 'acknowledged'] } } })
      : 0;
