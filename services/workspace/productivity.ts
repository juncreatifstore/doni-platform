...
export async function getProductivityOverview() {
  ...
  const rows = users
    .map((u) => {
      ...
      return {
        ...
        role: u.role,
        department: departments.get(u.id) || null,
        departmentLabel: departmentLabel(departments.get(u.id) || null),
        open: open.length,
  ...
  const summary = {
    ...
    overdue: tasks.filter((t) => t.status !== 'DONE' && t.dueAt && new Date(t.dueAt).getTime() < now).length,
    blocked: tasks.filter((t) => t.status === 'BLOCKED').length,
    unassigned: tasks.filter((t) => t.status !== 'DONE' && !t.assigneeId).length,
    doneToday: tasks.filter((t) => t.status === 'DONE' && new Date(t.updatedAt) >= startToday).length,
  };
  const departmentMap = new Map<
    string,
    { department: string; open: number; overdue: number; blocked: number; urgent: number }
  >();
  for (const t of tasks.filter((t) => t.status !== 'DONE')) {
    const key = t.department;
    const d = departmentMap.get(key) || { department: key, open: 0, overdue: 0, blocked: 0, urgent: 0 };
    d.open++;
    if (t.dueAt && new Date(t.dueAt).getTime() < now) d.overdue++;
    if (t.status === 'BLOCKED') d.blocked++;
    if (t.priority === 'URGENT') d.urgent++;
    departmentMap.set(key, d);
  }
