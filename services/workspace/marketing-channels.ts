...
function stats(values: number[], openEpisodes: number): ResponseStats {
  ...
  const within = values.filter((v) => v <= RESPONSE_TARGET_MINUTES).length;
  return {
    samples: values.length,
    averageMinutes: avg === null ? null : Math.round(avg * 10) / 10,
    medianMinutes: percentile(values, 0.5),
    p90Minutes: percentile(values, 0.9),
    withinTarget: within,
    withinTargetPct: values.length ? Math.round((within / values.length) * 1000) / 10 : null,
    openEpisodes,
  };
}
  ...
  for (const arr of byConversation.values()) {
    ...
    for (const m of arr) {
      if (m.direction === 'INBOUND' && m.senderType === 'CUSTOMER') {
        if (!pendingStart) pendingStart = m.createdAt;
        continue;
      }
      if (pendingStart && m.direction === 'OUTBOUND' && acceptedSenderTypes.includes(m.senderType)) {
        durations.push(Math.max(0, (m.createdAt.getTime() - pendingStart.getTime()) / 60000));
        pendingStart = null;
      }
    }
...
export async function getMarketingChannelPerformance(days = 30) {
  ...
  const rows = MARKETING_SOURCES.map((source) => {
    ...
    const qualified = sourceLeads.filter((x) =>
      ['QUALIFIED', 'HOT', 'QUOTE_SENT', 'FOLLOW_UP', 'CONVERTED'].includes(x.status),
    ).length;
    ...
    for (const s of sales) revenue.set(s.currency, (revenue.get(s.currency) || 0) + s.amount);
    return {
      source,
      ...
      leadToSalePct: sourceLeads.length ? Math.round((sales.length / sourceLeads.length) * 1000) / 10 : null,
      revenueByCurrency: [...revenue.entries()].map(([currency, amount]) => ({ currency, amount })),
    };
  }).sort((a, b) => b.attributedSales - a.attributedSales || b.leads - a.leads);
  const whatsappMessages = messages.filter(
    (m) => m.senderType === 'CUSTOMER' || m.senderType === 'BOT' || m.senderType === 'AGENT',
  );
  return {
    periodDays: days,
    ...
    channels: rows,
    whatsapp: {
      firstDoniResponse: responseEpisodes(whatsappMessages, ['BOT', 'AGENT']),
      humanResponse: responseEpisodes(whatsappMessages, ['AGENT']),
    },
