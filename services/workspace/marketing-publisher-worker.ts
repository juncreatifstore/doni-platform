...
function requiresFarePreflight(item:Item){const source=String(item.source||'').toLowerCase();return source.includes('live offers engine')||source.includes('abandoned booking recovery')||source.includes('v2.3 live offers')||source.includes('v2.2 abandoned');}
...
export async function runMarketingPublisherWorker(limit=20){
 ...
 if(policy.globalPause){await heartbeat({due:0,paused:true,reason:'global_pause'});return{due:0,sent:0,dryRun:0,blocked:0,failed:0,skipped:0,deferred:0,reviewRequired:0,paused:true,results:[],ranAt:new Date().toISOString()};}
 ...
 const stats={due:due.length,sent:0,dryRun:0,blocked:0,failed:0,skipped:0,deferred:0,reviewRequired:0};const results:Array<Record<string,unknown>>=[];await heartbeat({due:due.length,maxPerRun:policy.maxPerRun,maxPerHour:policy.maxPerHour,maxPerRecipient24h:policy.maxPerRecipient24h});
 for(const item of due){if(await alreadySent(item.id)){stats.skipped++;results.push({id:item.id,status:'SKIPPED',reason:'idempotent_already_sent'});continue;}if(!(await acquireLock(item.id))){stats.skipped++;results.push({id:item.id,status:'SKIPPED',reason:'locked'});continue;}try{const channel=String(item.channelSuggestion||'').toLowerCase();if(!channel.includes('whatsapp')){stats.blocked++;await logEvent(item,'BLOCKED',{reason:'channel_not_connected'});results.push({id:item.id,status:'BLOCKED',reason:'channel_not_connected'});continue;}const to=await resolveRecipient(item);if(!to){stats.blocked++;await logEvent(item,'BLOCKED',{reason:'recipient_not_found'});results.push({id:item.id,status:'BLOCKED',reason:'recipient_not_found'});continue;}const consentGate=await evaluateMarketingConsent(to);if(!consentGate.allowed){stats.blocked++;await logEvent(item,'BLOCKED',{reason:consentGate.reason,to,consentStatus:consentGate.consent.status,consentSource:consentGate.consent.source});results.push({id:item.id,status:'BLOCKED',reason:consentGate.reason});continue;}const gate=await evaluatePublisherPolicy(to);if(!gate.allowed){stats.deferred++;await logEvent(item,'DEFERRED',{reason:gate.reason,retryAfter:gate.retryAfter||null,to});results.push({id:item.id,status:'DEFERRED',reason:gate.reason,retryAfter:gate.retryAfter||null});continue;}
  if(requiresFarePreflight(item)){
   if(!item.conversationId){stats.reviewRequired++;await logEvent(item,'REVIEW_REQUIRED',{reason:'fare_preflight_conversation_missing'});results.push({id:item.id,status:'REVIEW_REQUIRED',reason:'fare_preflight_conversation_missing'});continue;}
   const preflight=await verifyConversationOffer(item.conversationId);
   if(!preflight){stats.reviewRequired++;await logEvent(item,'REVIEW_REQUIRED',{reason:'fare_preflight_offer_missing',conversationId:item.conversationId});results.push({id:item.id,status:'REVIEW_REQUIRED',reason:'fare_preflight_offer_missing'});continue;}
   if(preflight.status!=='VERIFIED'){
    stats.reviewRequired++;
    await logEvent(item,'REVIEW_REQUIRED',{reason:`fare_preflight_${preflight.status.toLowerCase()}`,preflightStatus:preflight.status,provider:preflight.provider,oldPrice:preflight.oldPrice,newPrice:preflight.price,currency:preflight.currency,verifiedAt:preflight.verifiedAt,expiresAt:preflight.expiresAt||null,preflightReason:preflight.reason||null});
    results.push({id:item.id,status:'REVIEW_REQUIRED',reason:`fare_preflight_${preflight.status.toLowerCase()}`,oldPrice:preflight.oldPrice,newPrice:preflight.price,currency:preflight.currency});continue;
   }
   await logEvent(item,'PREFLIGHT_VERIFIED',{provider:preflight.provider,price:preflight.price,currency:preflight.currency,verifiedAt:preflight.verifiedAt,expiresAt:preflight.expiresAt||null});
  }
  const delivery=await sendWhatsAppText(to,item.message);if(delivery.dryRun||!delivery.sent){stats.dryRun++;await logEvent(item,'DRY_RUN',{to});results.push({id:item.id,status:'DRY_RUN'});continue;}const providerMessageId=String((delivery as any)?.response?.messages?.[0]?.id||'');if(providerMessageId)await registerMarketingOutbound({itemId:item.id,conversationId:item.conversationId||null,recipient:to,providerMessageId}).catch(()=>null);const marked=await markAutopilotSent(item.id,{id:'publisher-worker',username:'publisher-worker',role:'SYSTEM'});if(!marked.ok){stats.failed++;await logEvent(item,'FAILED',{reason:marked.reason,providerMessageId:providerMessageId||null});results.push({id:item.id,status:'FAILED',reason:marked.reason});continue;}stats.sent++;await logEvent(item,'SENT',{to,idempotencyKey:`publisher:${item.id}`,consentStatus:consentGate.consent.status,providerMessageId:providerMessageId||null,conversationId:item.conversationId||null});results.push({id:item.id,status:'SENT',providerMessageId:providerMessageId||null});}catch(error){const reason=error instanceof Error?error.message:'publisher_failed';stats.failed++;await logEvent(item,'FAILED',{reason});results.push({id:item.id,status:'FAILED',reason});}finally{await releaseLock(item.id);}}
