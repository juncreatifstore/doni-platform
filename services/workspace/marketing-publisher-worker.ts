...
async function alreadySent(itemId:string){const rows=await db.appSetting.findMany({where:{category:EVENT_CATEGORY,key:{startsWith:EVENT_PREFIX}},orderBy:{createdAt:'desc'},take:300});return rows.some(r=>{const v=r.value as any;return v?.itemId===itemId&&v?.status==='SENT';});}
