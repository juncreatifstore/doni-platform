...
export async function runWorkspaceAutomation(){
 ...
 const [reviews,tickets,incidents,postBooking,deliveryFailures]=await Promise.all([
  ...
  prisma.flightIncident.findMany({where:{status:{in:['open','acknowledged']}},orderBy:{createdAt:'asc'},take:100,select:{id:true,type:true,severity:true,status:true,createdAt:true,tracking:{select:{ticketReference:true,airlineCode:true,flightNumber:true,origin:true,destination:true}}}}),
 ...
 for(const i of incidents){const age=ageMinutes(i.createdAt);const flight=i.tracking?`${i.tracking.airlineCode}${i.tracking.flightNumber} ${i.tracking.origin}→${i.tracking.destination}`:'Vol';candidates.push({key:`flight-incident:${i.id}`,title:`Incident ${flight}`,description:`${i.type||'incident'} · ${i.severity||'standard'} · billet ${i.tracking?.ticketReference||'non lié'}`,department:'FLIGHT_OPS',priority:i.severity==='critical'?'URGENT':age>=30?'URGENT':'HIGH',dueAt:dueFrom(i.createdAt,15),entityType:'FlightIncident',entityId:i.id,sourceStatus:i.status});}
