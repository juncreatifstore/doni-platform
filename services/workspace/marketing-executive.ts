...
function recent(iso: string, days: number) {
  return new Date(iso).getTime() >= Date.now() - days * 86400000;
}
export async function getMarketingExecutiveCockpit() {
  const [leads, content, campaigns, partnerships, studio, attributions, plans] = await Promise.all([
    listMarketingLeads(),
  ...
  const contentUpcoming = content.filter(
    (x) => x.scheduledAt && new Date(x.scheduledAt).getTime() >= Date.now() && x.status !== 'PUBLISHED',
  ).length;
  ...
  const activePartnerships = partnerships.filter(
    (x) => x.status === 'ACTIVE' || x.status === 'SIGNED',
  ).length;
  const overduePartnerships = partnerships.filter(
    (x) =>
      x.nextActionAt &&
      new Date(x.nextActionAt).getTime() < Date.now() &&
      !['ACTIVE', 'SIGNED', 'LOST'].includes(x.status),
  ).length;
  const activePlans = plans
    .filter((x) => x.status === 'ACTIVE' || x.status === 'REVIEW')
    .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
  const revenueByCurrency = new Map<string, { currency: string; revenue: number; sales: number }>();
  for (const a of attributions) {
    const x = revenueByCurrency.get(a.currency) || { currency: a.currency, revenue: 0, sales: 0 };
    x.revenue += a.amount;
    x.sales += 1;
    revenueByCurrency.set(a.currency, x);
  }
  ...
  for (const a of attributions) sourceMap.set(a.source, (sourceMap.get(a.source) || 0) + 1);
  ...
  return {
    summary: {
      ...
      activePartnerships,
      overduePartnerships,
      attributedSales: attributions.length,
    },
    revenueByCurrency: [...revenueByCurrency.values()],
    campaignRows,
    sources,
    plans: activePlans.slice(0, 6),
  };
}
