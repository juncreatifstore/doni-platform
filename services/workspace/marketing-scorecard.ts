...
export async function getMarketingScorecard(){
 const [base,content,reviews,partnerships,performance,channels]=await Promise.all([
  getMarketingPhase1Dashboard(),listMarketingContent(),listMarketingReviews(),listMarketingPartnerships(),getMarketingPerformance(),getMarketingChannelPerformance(30)
 ]);
 ...
 const realCurrencyPerformance=performance.currencyTotals.map(x=>({currency:x.currency,spend:x.spend,revenue:x.revenue,sales:x.sales,roas:x.roas,roi:x.roi,targetMet:x.roas!==null&&x.roas>=MARKETING_GOALS.monthly.roiTarget}));
 const goalRows={daily:[
  ...
  {key:'daily-published',label:'Contenus publiés',value:base.metrics.dayPublished,target:MARKETING_GOALS.daily.publicationsMin,progress:pct(base.metrics.dayPublished,MARKETING_GOALS.daily.publicationsMin)}],weekly:[
  ...
  {key:'weekly-reviews',label:'Avis clients',value:weekReviews,target:MARKETING_GOALS.weekly.reviews,progress:pct(weekReviews,MARKETING_GOALS.weekly.reviews)}],monthly:[
 ...
 const all=[...goalRows.daily,...goalRows.weekly,...goalRows.monthly];const met=all.filter(x=>x.progress>=100).length;
 return {goals:MARKETING_GOALS,goalRows,summary:{tracked:all.length,met,atRisk:all.length-met,completion:all.length?Math.round(all.reduce((a,x)=>a+x.progress,0)/all.length):0},response:{targetMinutes:channels.targetMinutes,periodDays:channels.periodDays,firstDoni:channels.whatsapp.firstDoniResponse,human:channels.whatsapp.humanResponse},realCurrencyPerformance,sourcePerformance:performance.sources.slice(0,8)};
}
