...
export async function getReservationOverview(reference?: string, scope: DataScope = { mode: 'global' }) {
  ...
  for (const t of tickets) {
    ...
    const tracking = await (db as any).flightTracking
      .findMany({ where: { ticketReference: t.reference }, orderBy: { scheduledDeparture: 'asc' } })
      .catch(() => []);
    const nextDeparture =
      (payload.segments || [])
        .map((s: any) => new Date(s.departureAt || 0))
        .filter((d: Date) => !Number.isNaN(d.getTime()) && d.getTime() > Date.now())
    ...
    const trackingRows = tracking.map((x: any) => ({
      ...
      estimatedDeparture: x.estimatedDeparture,
    ...
    const hasDisruption = trackingRows.some(
      (x: any) =>
        ['delayed', 'cancelled', 'diverted'].includes(String(x.flightStatus || '').toLowerCase()) ||
        Number(x.delayMinutes || 0) >= 15,
    );
    rows.push({
      ...
      checkinEligible,
      checkinWindow: checkinEligible
        ? 'OPEN'
        : hoursToDeparture !== null && hoursToDeparture > 48
          ? 'NOT_OPEN_YET'
          : 'CLOSED',
