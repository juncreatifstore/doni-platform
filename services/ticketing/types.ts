...
export function normalizeTicketPayload(v: unknown): TicketPayload {
  const p = (v && typeof v === 'object' ? v : {}) as any;
  const srcP = Array.isArray(p.passengers) ? p.passengers : Array.isArray(p.travelers) ? p.travelers : [];
  const passengers = srcP.map((x: any) => ({
  ...
  const srcS = Array.isArray(p.segments) ? p.segments : [];
  const segments = srcS.map((x: any) => ({
    ...
    pnr: x.pnr ?? x.bookingReference ?? x.booking_reference ?? x.locator ?? '',
    bookingReference: x.bookingReference ?? x.booking_reference ?? x.pnr ?? x.locator ?? '',
  }));
  return { ...p, passengers, segments, total: p.total ?? p.amount, offerId: p.offerId ?? p.offer_id };
}
