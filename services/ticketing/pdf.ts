...
export type EticketBranding = {
  supportEmail?: string;
  supportPhone?: string;
  verificationUrl?: string;
  appName?: string;
};
const NAVY = rgb(0.015, 0.12, 0.27),
  NAVY2 = rgb(0.025, 0.2, 0.38),
  BLUE = rgb(0.05, 0.42, 0.67),
  SKY = rgb(0.83, 0.93, 0.98),
  LIGHT = rgb(0.965, 0.978, 0.99),
  TEXT = rgb(0.06, 0.11, 0.19),
  MUTED = rgb(0.34, 0.4, 0.48),
  BORDER = rgb(0.78, 0.85, 0.91),
  WHITE = rgb(1, 1, 1);
...
export async function buildEticketPdf(
  ticket: {
    reference: string;
    pnr: string | null;
  ...
  const draw = (s: string, x: number, y: number, size = 10, b = false, color = TEXT) =>
    page.drawText(s, { x, y, size, font: b ? bold : regular, color });
  ...
  page.drawLine({ start: { x: 297, y: 660 }, end: { x: 297, y: 724 }, thickness: 0.7, color: BORDER });
  draw('ISSUED', 48, 669, 7, true, MUTED);
  draw(ticket.issuedAt ? ticket.issuedAt.toISOString().slice(0, 10) : '-', 98, 668, 9, true, TEXT);
  draw('SEGMENTS', 315, 669, 7, true, MUTED);
  draw(String(payload.segments?.length || 0), 369, 668, 9, true, TEXT);

  let y = 617;
  drawSectionTitle(page, bold, 'PASSENGER', y, 'P');
  y -= 31;
  const passengers = payload.passengers || [];
  if (passengers.length === 0) {
    roundedPanel(page, 48, y - 5, 499, 28, LIGHT, BORDER, 0.6);
    draw('No passenger data', 62, y + 4, 9, false, MUTED);
    y -= 36;
  } else
    for (const [i, p] of passengers.entries()) {
      roundedPanel(page, 48, y - 5, 499, 30, LIGHT, BORDER, 0.6);
      draw(
      ...
      draw(clean(p.type, 'Adult'), 418, y + 4, 9, false, MUTED);
      y -= 38;
    }

  y -= 2;
  drawSectionTitle(page, bold, 'ITINERARY', y, 'F');
  y -= 22;
  ...
  roundedPanel(page, 42, infoY, 245, 72, LIGHT, BORDER, 0.7);
  draw('BOOKING VERIFICATION', 58, infoY + 53, 9, true, NAVY);
  if (branding.verificationUrl) {
    try {
      const qr = await QRCode.toBuffer(branding.verificationUrl, {
        type: 'png',
        width: 180,
        margin: 1,
      ...
      page.drawImage(img, { x: 58, y: infoY + 10, width: 40, height: 40 });
      draw('Scan to open your secure', 108, infoY + 35, 7.5, false, MUTED);
      draw('e-ticket verification link', 108, infoY + 23, 7.5, false, MUTED);
      draw(ticket.reference, 108, infoY + 10, 7.5, true, TEXT);
    } catch {
      draw('Use your DONI reference to verify', 58, infoY + 31, 8, false, MUTED);
  ...
  roundedPanel(page, 308, infoY, 245, 72, LIGHT, BORDER, 0.7);
  draw('NEED HELP?', 324, infoY + 53, 9, true, NAVY);
  let helpY = infoY + 35;
  if (branding.supportEmail) {
    draw(branding.supportEmail, 324, helpY, 8, false, TEXT);
    helpY -= 13;
  }
  if (branding.supportPhone) {
    draw(branding.supportPhone, 324, helpY, 8, false, TEXT);
    helpY -= 13;
  }
  if (!branding.supportEmail && !branding.supportPhone)
    draw('Contact DONI Travel support', 324, helpY, 8, false, TEXT);
  draw('Keep this e-ticket and your travel documents.', 324, infoY + 10, 7, false, MUTED);

  page.drawLine({ start: { x: 44, y: 83 }, end: { x: 268, y: 83 }, thickness: 0.7, color: BORDER });
  page.drawLine({ start: { x: 327, y: 83 }, end: { x: 551, y: 83 }, thickness: 0.7, color: BORDER });
  draw('AIR', 281, 79, 7, true, BLUE);
  draw('Please verify names, dates and travel documents before departure.', 106, 62, 8, false, MUTED);
  draw('Generated by ', 220, 43, 8, false, MUTED);
  draw(appName, 271, 43, 8, true, NAVY);
...
function drawPremiumHeader(page: PDFPage, bold: PDFFont, appName: string) {
  ...
  page.drawRectangle({ x: 365, y: 744, width: 230, height: 98, color: NAVY2, opacity: 0.42 });
  page.drawLine({
    start: { x: 378, y: 759 },
    end: { x: 548, y: 817 },
    thickness: 2.4,
  ...
  page.drawText(appName.toUpperCase(), { x: 40, y: 798, size: 26, font: bold, color: WHITE });
...
function drawPremiumSegment(
  ...
  const x = 42,
    ...
    h = 104,
    top = y,
    pnr = clean(s.pnr || s.bookingReference || globalPnr).toUpperCase();
  roundedPanel(page, x, top - h, w, h, WHITE, BORDER, 0.9);
  ...
  page.drawLine({
    start: { x: x + 1, y: top - 25 },
    end: { x: x + w - 1, y: top - 25 },
    thickness: 0.7,
    color: BORDER,
  });
  ...
  page.drawText(`${clean(s.origin, '---')}  ->  ${clean(s.destination, '---')}`, {
    x: x + 124,
    y: top - 51,
    size: 15,
    font: bold,
  ...
  const originName = clean(s.originName, '');
  const destinationName = clean(s.destinationName, '');
  if (originName || destinationName) {
    page.drawText(originName, { x: x + 124, y: top - 64, size: 6.8, font: regular, color: MUTED });
    page.drawText(destinationName, { x: x + 256, y: top - 64, size: 6.8, font: regular, color: MUTED });
  }
  page.drawText('DEPART', { x: x + 124, y: top - 78, size: 7, font: bold, color: MUTED });
  page.drawText(fmtDate(s.departureAt), { x: x + 124, y: top - 92, size: 8, font: regular, color: TEXT });
  page.drawText(fmtTime(s.departureAt), { x: x + 197, y: top - 92, size: 10, font: bold, color: NAVY });
  page.drawText('>', { x: x + 260, y: top - 90, size: 12, font: bold, color: BLUE });
  page.drawText('ARRIVE', { x: x + 292, y: top - 78, size: 7, font: bold, color: MUTED });
  page.drawText(fmtDate(s.arrivalAt), { x: x + 292, y: top - 92, size: 8, font: regular, color: TEXT });
  page.drawText(fmtTime(s.arrivalAt), { x: x + 365, y: top - 92, size: 10, font: bold, color: NAVY });
  page.drawText(`PNR ${pnr}`, { x: x + 18, y: top - 93, size: 7.5, font: bold, color: BLUE });
  return top - h - 10;
}
...
function circleBadge(page: PDFPage, bold: PDFFont, x: number, y: number, text: string, color: any, bg?: any) {
  page.drawCircle({
    ...
    size: 11,
    color: bg || color,
    borderColor: bg ? color : undefined,
    borderWidth: bg ? 1 : 0,
  });
