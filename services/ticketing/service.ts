...
function payloadOf(v: unknown): TicketPayload {
  return normalizeTicketPayload(v);
}
...
export async function getTicketQueue(scope: DataScope = { mode: 'global' }) {
  ...
  return rows.map((t: any) => {
    const payload = payloadOf(t.payload);
    return {
      id: t.id,
      ...
      passengerCount: Array.isArray(payload.passengers) ? payload.passengers.length : 0,
...
export async function getRecentlyIssued(scope: DataScope = { mode: 'global' }) {
  const rows = await prisma.ticket.findMany({
    where: { status: 'ISSUED', ...conversationCountryWhere(scope) },
    include: { conversation: true },
  ...
  return rows.map((t: any) => {
    ...
    return {
      ...
      passengerCount: payload.passengers?.length || 0,
...
export async function issueManual(input: {
  reference: string;
  pnr: string;
  ticketNumber?: string;
  agent?: string;
}) {
  const ticket = await prisma.ticket.findUnique({ where: { reference: input.reference } });
  ...
  if (!/^[A-Z0-9]{4,12}$/.test(pnr)) throw new Error('invalid_pnr');
  const updated = await prisma.$transaction(async (tx: any) => {
    await tx.ticket.update({ where: { id: ticket.id }, data: { status: 'ISSUING' } });
    await tx.ticketEvent.create({
      data: {
        ticketId: ticket.id,
        eventType: 'manual_issue_started',
        status: 'ISSUING',
        actor: input.agent || 'portal',
      },
    });
    const done = await tx.ticket.update({
      where: { id: ticket.id },
      data: {
        status: 'ISSUED',
        pnr,
        ticketNumber: input.ticketNumber?.trim() || null,
        issuedBy: input.agent || 'portal',
        issuedAt: new Date(),
      },
    });
    await tx.ticketEvent.create({
      data: {
        ticketId: ticket.id,
        eventType: 'manual_issue_completed',
        status: 'ISSUED',
        actor: input.agent || 'portal',
        details: { pnr, ticketNumber: input.ticketNumber || null },
      },
    });
    return done;
  });
...
export async function setSegmentPnrs(input: { reference: string; pnrs: string[]; agent?: string }) {
  ...
  const next = {
    ...payload,
    segments: segments.map((s: any, i: number) => ({ ...s, pnr: pnrs[i], bookingReference: pnrs[i] })),
  };
  const updated = await prisma.ticket.update({ where: { id: ticket.id }, data: { payload: next as any } });
  await prisma.ticketEvent.create({
    data: {
      ticketId: ticket.id,
      eventType: 'segment_pnrs_updated',
      status: 'ISSUED',
      actor: input.agent || 'portal',
      details: { pnrs },
    },
