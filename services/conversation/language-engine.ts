...
const WEAK: Record<DetectableLanguage, string[]> = {
  ht: ['ak', 'nan', 'pa', 'pwen', 'ki sa'],
  fr: ['le', 'la', 'les', 'avec', 'pour', 'dans'],
  es: ['el', 'la', 'los', 'con', 'para', 'es'],
  en: ['the', 'with', 'for', 'in', 'is', 'a'],
};
const HT_WORDS = [
  'mwen',
  'kijan',
...
const ONE_WORD: Record<string, DetectableLanguage | null> = {
  oui: 'fr',
  non: 'fr',
  sí: 'es',
  si: null,
...
export function detectLanguage(input: string): DetectableLanguage | null {
  ...
  (Object.keys(STRONG) as DetectableLanguage[]).forEach((lang) =>
    STRONG[lang].forEach((m) => {
      if (containsWord(text, m)) scores[lang] += 5;
    }),
  );
  (Object.keys(WEAK) as DetectableLanguage[]).forEach((lang) =>
    WEAK[lang].forEach((m) => {
      if (containsWord(text, m)) scores[lang] += 1;
    }),
  );
...
export function sessionLanguage(session: Pick<ConversationSession, 'lockedLanguage'>): DoniLanguage {
  return session.lockedLanguage ?? 'es';
}
export function translate(
  session: Pick<ConversationSession, 'lockedLanguage'>,
  values: Partial<Record<DoniLanguage, string>>,
): string {
...
export function nativeLanguageName(lang: DoniLanguage) {
  return (
    ({ fr: 'Français', en: 'English', es: 'Español', ht: 'Kreyòl', fil: 'Filipino', ceb: 'Bisaya' } as const)[
      lang
    ] ?? lang
  );
}
