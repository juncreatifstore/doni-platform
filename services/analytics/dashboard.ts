...
async function resolveScope(scope?: DataScope): Promise<DataScope> {
  if (scope) return scope;
  const user = await getCurrentUser();
  return user ? dataScopeForUser(user) : { mode: 'none' };
}
...
export async function getOverviewMetrics(inputScope?: DataScope) {
  ...
  const conversationScope = countryWhere(scope),
    linkedScope = conversationCountryWhere(scope);
  const [
    active,
    agentRequired,
    stalled,
    ...
    db.doniConversation.count({
      where: { status: 'ACTIVE', updatedAt: { lt: staleAt }, ...conversationScope },
    }),
    ...
    db.payment.findMany({
      where: { status: 'PAID', updatedAt: { gte: last24h }, ...linkedScope },
      select: { currency: true, amount: true },
    }),
    db.doniConversation.findMany({
  ...
  for (const row of paidRows)
    revenueByCurrency.set(row.currency, (revenueByCurrency.get(row.currency) || 0) + num(row.amount));
  return {
    active,
    agentRequired,
    stalled,
    ...
    issued24h,
    deliveryFailures,
    revenue24h: [...revenueByCurrency].map(([currency, amount]) => ({ currency, amount })),
    recentConversations,
  };
}
...
export async function getFinanceMetrics(inputScope?: DataScope) {
  ...
  const [paid7d, paidPrev7d, statusGroups, providerRows, recentPayments] = await Promise.all([
    db.payment.findMany({
      where: { status: 'PAID', updatedAt: { gte: last7d }, ...paymentScope },
      select: { currency: true, amount: true, provider: true, updatedAt: true },
    }),
    db.payment.findMany({
      where: { status: 'PAID', updatedAt: { gte: previous7d, lt: last7d }, ...paymentScope },
      select: { currency: true, amount: true },
    }),
    db.payment.groupBy({ by: ['status'], where: paymentScope, _count: { _all: true } }),
    db.payment.findMany({
      where: { status: 'PAID', updatedAt: { gte: last7d }, ...paymentScope },
      select: { provider: true, currency: true, amount: true },
    }),
    db.payment.findMany({
      where: paymentScope,
      orderBy: { updatedAt: 'desc' },
  ...
  const byProviderCurrency = new Map<
    string,
    { provider: string; currency: string; amount: number; count: number }
  >();
  for (const r of paid7d) {
    const cur = byCurrency.get(r.currency) || { amount: 0, count: 0 };
    cur.amount += num(r.amount);
    cur.count++;
    byCurrency.set(r.currency, cur);
    if (r.updatedAt >= last24h) {
      const d = byCurrency24h.get(r.currency) || { amount: 0, count: 0 };
      d.amount += num(r.amount);
      d.count++;
      byCurrency24h.set(r.currency, d);
    }
  }
  for (const r of paidPrev7d)
    prevByCurrency.set(r.currency, (prevByCurrency.get(r.currency) || 0) + num(r.amount));
  for (const r of providerRows) {
    const k = `${r.provider}::${r.currency}`;
    const d = byProviderCurrency.get(k) || {
      provider: r.provider,
      currency: r.currency,
      amount: 0,
      count: 0,
    ...
    d.amount += num(r.amount);
    d.count++;
    byProviderCurrency.set(k, d);
  }
  const currencyTotals = [...byCurrency].map(([currency, v]) => ({
    currency,
    ...v,
    previousAmount: prevByCurrency.get(currency) || 0,
    changePct:
      (prevByCurrency.get(currency) || 0) > 0
        ? Math.round(
            ((v.amount - (prevByCurrency.get(currency) || 0)) / (prevByCurrency.get(currency) || 1)) * 1000,
          ) / 10
        : null,
  })) as Array<CurrencyTotal & { previousAmount: number; changePct: number | null }>;
  const dayTotals = [...byCurrency24h].map(([currency, v]) => ({ currency, ...v })) as CurrencyTotal[];
  const providerTotals = [...byProviderCurrency.values()].sort((a, b) => b.amount - a.amount) as Array<
    ProviderTotal & { currency: string }
  >;
  return {
    currencyTotals,
    dayTotals,
...
const FUNNEL = [
  ...
  [
    ...
    [
      'segment_origin',
      'segment_airport_choice_origin',
      'segment_destination',
      'segment_airport_choice_destination',
    ],
  ],
...
export async function getFlowMetrics(inputScope?: DataScope) {
  const scope = await resolveScope(inputScope);
  const stale10 = since(10 * 60 * 1000),
    stale30 = since(30 * 60 * 1000),
    last24 = since(DAY);
  const conversationScope = countryWhere(scope),
    linkedScope = conversationCountryWhere(scope);
  const [segmentGroups, activeRows, started24, paid24, issued24] = await Promise.all([
    db.doniConversation.groupBy({
      by: ['currentSegment'],
      where: { status: 'ACTIVE', ...conversationScope },
      _count: { _all: true },
    }),
    db.doniConversation.findMany({
      where: { status: 'ACTIVE', ...conversationScope },
  ...
  const funnel = FUNNEL.map(([key, label, segments]) => ({
    key,
    label,
    count: segments.reduce((n, s) => n + (segmentMap.get(s) || 0), 0),
  }));
  const unknown = segmentGroups
    .filter((x) => !FUNNEL.some(([, , ss]) => (ss as readonly string[]).includes(x.currentSegment)))
    .map((x) => ({ segment: x.currentSegment, count: x._count._all }))
    .sort((a, b) => b.count - a.count);
  const stalled = activeRows
    .filter((r) => r.updatedAt < stale10)
    .map((r) => ({
      ...r,
      stalledMinutes: Math.max(0, Math.floor((Date.now() - r.updatedAt.getTime()) / 60000)),
    }));
  const critical = stalled.filter((r) => r.updatedAt < stale30 || r.agentRequired);
  return {
    funnel,
    unknown,
    stalled: stalled.slice(0, 30),
    criticalCount: critical.length,
    started24,
    paid24,
    issued24,
    paymentConversionPct: pct(paid24, started24),
    issueConversionPct: pct(issued24, started24),
