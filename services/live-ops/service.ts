...
function assertLiveAccess(user: SafeUser) {
  if (!canAccessDepartments(user, LIVE_DEPARTMENTS as any)) throw new Error('forbidden_department');
}
async function assertConversationScope(id: string, user: SafeUser) {
  assertLiveAccess(user);
  const scope = dataScopeForUser(user);
  const row = await db.doniConversation.findFirst({
    where: { id, ...countryWhere(scope) },
    select: { id: true },
  });
...
export async function listLiveConversations(limit = 100, user?: SafeUser) {
  ...
  const rows = await db.doniConversation.findMany({
    ...
    include: {
      assignedAgent: { select: { id: true, username: true, fullName: true } },
  ...
  return rows.map((r) => ({
    ...
    language: r.lockedLanguage || r.language,
    agentRequired: r.agentRequired,
  ...
  const row = await db.doniConversation.findUnique({
    ...
    include: {
      ...
      messages: {
        ...
        take: 500,
        include: { senderUser: { select: { username: true, fullName: true } } },
      },
...
export async function recordMessage(input: {
  ...
  if (input.providerMessageId) {
    const exists = await db.conversationMessage
      .findUnique({ where: { providerMessageId: input.providerMessageId } })
      .catch(() => null);
...
export async function takeoverConversation(id: string, user: SafeUser) {
  ...
  const current = await db.doniConversation.findUnique({
    where: { id },
    select: { id: true, status: true, assignedAgentId: true, waId: true },
  });
  if (!current) throw new Error('conversation_not_found');
  if (current.status === 'AGENT_HOLD' && current.assignedAgentId && current.assignedAgentId !== user.id)
  ...
  const row = await db.doniConversation.update({
    ...
    data: {
      ...
      agentTakenOverAt: new Date(),
      agentReleasedAt: null,
    },
  });
  await recordMessage({
    conversationId: id,
    direction: 'OUTBOUND',
    senderType: 'SYSTEM',
    text: `Conversation prise en charge par ${user.fullName || user.username}.`,
    senderUserId: user.id,
    metadata: { event: 'takeover' },
  });
  await audit({
    userId: user.id,
    action: 'conversation.takeover',
    entity: 'DoniConversation',
    entityId: id,
    metadata: { waId: current.waId, country: user.country || null },
  });
  return row;
}
...
export async function releaseConversation(id: string, user: SafeUser) {
  ...
  if (!current) throw new Error('conversation_not_found');
  if (current.assignedAgentId && current.assignedAgentId !== user.id && user.role === 'AGENT')
    throw new Error('conversation_assigned_to_other_agent');
  const row = await db.doniConversation.update({
    where: { id },
    data: { status: 'ACTIVE', agentRequired: false, assignedAgentId: null, agentReleasedAt: new Date() },
  });
  await recordMessage({
    conversationId: id,
    direction: 'OUTBOUND',
    senderType: 'SYSTEM',
    text: `Conversation rendue à DONI par ${user.fullName || user.username}.`,
    senderUserId: user.id,
  ...
  if (handler) {
    const session = {
      ...
      lockedLanguage: row.lockedLanguage as any,
    ...
    if (prompt) {
      resumeDelivery = await sendWhatsAppText(row.waId, prompt);
      const pid = (resumeDelivery as any)?.response?.messages?.[0]?.id ?? null;
      await recordMessage({
        conversationId: id,
        direction: 'OUTBOUND',
        senderType: 'BOT',
        ...
        metadata: { event: 'resume_prompt', delivery: resumeDelivery },
      });
    }
  }
  await audit({
    ...
    entity: 'DoniConversation',
    entityId: id,
    metadata: { waId: current.waId, resumePrompt: Boolean(handler), country: user.country || null },
  });
...
export async function sendAgentMessage(id: string, user: SafeUser, text: string) {
  ...
  if (clean.length > 4096) throw new Error('message_too_long');
  const c = await db.doniConversation.findUnique({
    where: { id },
    select: { id: true, waId: true, status: true, assignedAgentId: true },
  });
  if (!c) throw new Error('conversation_not_found');
  if (c.status !== 'AGENT_HOLD') throw new Error('takeover_required');
  if (c.assignedAgentId !== user.id && user.role === 'AGENT')
    throw new Error('conversation_assigned_to_other_agent');
  ...
  await db.doniConversation.update({
    where: { id },
    data: { lastMessageAt: new Date(), agentRequired: true },
  });
  await audit({
    userId: user.id,
    action: 'conversation.agent_message',
    entity: 'DoniConversation',
    entityId: id,
    metadata: { dryRun: (delivery as any)?.dryRun === true, country: user.country || null },
  });
