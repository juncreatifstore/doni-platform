...
export async function deliverDisruptionsForTicket(reference: string) {
  ...
  if (!alertsEnabled || !autoSendEnabled || !whatsappEnabled)
    throw new Error('automatic_delivery_gates_disabled');
  if (standardEnabled) throw new Error('standard_alerts_must_remain_disabled');
  const now = new Date();
  const rows: any[] = await (db as any).flightAlert
    .findMany({
      where: {
        status: 'pending',
        scheduledAt: { lte: now },
        alertType: { startsWith: 'disruption:' },
        tracking: { ticketReference: ref },
      },
  ...
  for (const a of rows) {
    try {
      ...
      sent++;
      results.push({ id: a.id, alertType: a.alertType, status: 'sent' });
    } catch (e) {
      const error = e instanceof Error ? e.message : 'send_failed';
      await (db as any).flightAlert
        .update({ where: { id: a.id }, data: { status: 'failed', error } })
        .catch(() => null);
      failed++;
      results.push({ id: a.id, alertType: a.alertType, status: 'failed', error });
    }
  }
  const remaining = await (db as any).flightAlert
    .count({
      where: {
        status: 'pending',
        alertType: { startsWith: 'disruption:' },
  ...
  return {
    ...
    failed,
    remaining,
    standardAlertsTouched: 0,
    dedupe: {
      sent: disruptionAlertState('sent'),
      pending: disruptionAlertState('pending'),
      failed: disruptionAlertState('failed'),
    },
  };
}
