...
function minorUnits(amount: number, currency: string | null | undefined) {
  const n = Number(amount);
  if (!Number.isFinite(n) || n <= 0) throw new Error('meta_ads_daily_budget_required');
  return String(Math.round(n * (ZERO_DECIMAL.has(String(currency || '').toUpperCase()) ? 1 : 100)));
}
...
export async function createPausedMetaAdSet(campaignId: string, userId: string) {
  const [campaign, config, status, accessToken, v] = await Promise.all([
  ...
  const json = await graphPost(
    `${encodeURIComponent(status.adAccountId)}/adsets`,
    accessToken,
    v || 'v23.0',
    {
  ...
  const remoteAdSetId = String(json?.id || '').trim();
  if (!remoteAdSetId) throw new Error('meta_ads_adset_id_missing');
