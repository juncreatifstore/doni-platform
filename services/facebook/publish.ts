...
export async function publishFacebookPagePost(input: FacebookPublishInput): Promise<FacebookPublishResult> {
  ...
  const message = whatsappLink
    ? `${input.message}\n\n📲 Vérifier le tarif et réserver avec DONI : ${whatsappLink}`
    : input.message;
  if (!enabled)
    return { sent: false, dryRun: true, pageId: pageId || null, message, whatsappLink, postId: null };
  ...
  const res = await fetch(`https://graph.facebook.com/${version}/${encodeURIComponent(pageId)}/feed`, {
    method: 'POST',
  ...
  return {
    ...
    message,
    whatsappLink,
    postId: String((json as any)?.id || '') || null,
    response: json,
  };
}
