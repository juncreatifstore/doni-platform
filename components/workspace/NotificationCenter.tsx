...
export function NotificationCenter() {
  ...
  async function mark(id: string) {
    await fetch(`/api/workspace/notifications/${id}`, {
      method: 'PATCH',
  ...
  return (
    <section className="notificationCenter">
      <div className="notificationHeader">
        <div>
          <span className="workspaceKicker">Centre interne</span>
          <h2>Notifications DONI</h2>
          <p>{unread} notification(s) non lue(s)</p>
        </div>
        <button className="btn" onClick={markAll} disabled={!unread}>
      ...
      {loading ? (
        ...
        <div className="notificationList">
          {rows.map((n) => (
            <article
              key={n.id}
              className={`notificationItem severity-${n.severity.toLowerCase()} ${n.readAt ? 'read' : 'unread'}`}
            >
              ...
              <div className="notificationActions">
                {n.href ? (
                  <Link className="btn" href={n.href} onClick={() => mark(n.id)}>
                    Ouvrir
                  </Link>
                ) : null}
                {!n.readAt ? (
        ...
        <div className="card muted">Aucune notification interne.</div>
      )}
    </section>
  );
}
