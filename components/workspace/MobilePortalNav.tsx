...
export function MobilePortalNav({ items }: { items: Item[] }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('doni:open-mobile-menu', onOpen);
    return () => window.removeEventListener('doni:open-mobile-menu', onOpen);
  }, []);
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="mobilePortalNav">
      <button
        className="mobileMenuButton"
        type="button"
        aria-expanded={open}
        aria-controls="mobile-portal-drawer"
      ...
      {open ? (
        ...
        >
          <aside id="mobile-portal-drawer" className="mobileNavDrawer" aria-label="Navigation mobile DONI">
            ...
            <nav className="mobileNavLinks" aria-label="Sections du portail">
              {items.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                ...
                >
                  ...
                  {item.badge && item.badge > 0 ? (
                    <span
                      className="mobileNavBadge"
                      aria-label={`${item.badge} élément${item.badge > 1 ? 's' : ''} à traiter`}
                    >
