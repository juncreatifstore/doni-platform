...
export function PortalQuickAccess({active,title,section}:{active:string;title:string;section?:string|null}){
 ...
 useEffect(()=>{const onKey=(e:KeyboardEvent)=>{if(e.altKey&&e.key.toLowerCase()==='q'){e.preventDefault();setOpen(v=>!v)}if(e.key==='Escape')setOpen(false)};const onOpen=()=>setOpen(true);window.addEventListener('keydown',onKey);window.addEventListener('doni:open-quick-access',onOpen);return()=>{window.removeEventListener('keydown',onKey);window.removeEventListener('doni:open-quick-access',onOpen)}},[]);
