...
const groups: NavGroup[] = [
  {
    label: 'Portail DONI',
    items: PORTAL_SECTIONS.map((section) => ({
      href: section.href,
      icon: section.icon,
      label: section.label,
...
function roleLabel(user: User) {
  const role = resolvedOrgRole(user);
  if (role === 'SUPER_ADMIN') return 'Super Admin';
  if (role === 'COUNTRY_ADMIN') return 'Admin pays';
  if (role === 'SECTION_MANAGER') return 'Responsable de section';
  if (role === 'PARTNER') return 'Partenaire';
  if (role === 'AGENT') return 'Agent';
  return user.role === 'SUPER_ADMIN' ? 'Super Admin' : user.role === 'ADMIN' ? 'Admin pays' : 'Agent';
}
function initials(user: User) {
  const source = (user.fullName || user.username || 'D').trim();
  return (
    source
      .split(/\s+/)
      .slice(0, 2)
...
function itemVisible(item: NavItem, user: User) {
  ...
  if (orgRole === 'SECTION_MANAGER' && (item.href === '/portal/sections/admin' || item.href === '/users'))
    return true;
...
function sectionBadges(w: Awaited<ReturnType<typeof getWorkHubMetrics>>) {
  return {
    '/portal/sections/sales': w.unassignedAttention + w.pendingPayments,
    '/portal/sections/customer-service':
      w.unassignedAttention + w.openPostBooking + w.activeCheckins + w.activeBaggage,
...
export async function DoniShell({
  children,
  title,
  active,
  user,
  ...
  const visibleGroups = groups
    .map((group) => ({ ...group, items: group.items.filter((item) => itemVisible(item, user)) }))
  ...
  const mobileItems = visibleGroups.flatMap((group) =>
    group.items.map((item) => ({
      href: item.href,
      icon: item.icon,
      label: item.label,
      active: active === item.href || portalActive === item.href,
    ...
    <div className="shell">
      ...
      <aside className="sidebar" aria-label="Navigation du portail">
        <div className="brand">
          ...
          <div className="brandText">
            <strong>DONI</strong>
            <small>Créatif Travel · Operations</small>
          </div>
        </div>
        <nav className="nav" aria-label="Navigation principale">
          {visibleGroups.map((group) => (
            <div className="navGroup" key={group.label}>
              ...
              {group.items.map((item) => {
                const isActive = active === item.href || portalActive === item.href;
                ...
                return (
                  ...
                  >
                    ...
                    {badge > 0 ? (
                      <span
                        className="navAttentionBadge"
                        aria-label={`${badge} élément${badge > 1 ? 's' : ''} à traiter`}
                      >
                        {badgeText(badge)}
                      </span>
                    ) : null}
      ...
      <main className="main" id="main-content" tabIndex={-1}>
        <header className="topbar">
          <div className="mobileTopbarLead">
            <MobilePortalNav items={mobileItems} />
            <div className="pageHeading">
              <div className="eyebrow">DONI Workspace · {scopeLabel}</div>
              <h1>{title}</h1>
              <div className="userLine">
                {user.fullName || user.username} · {roleLabel(user)}
              </div>
            </div>
          </div>
          <div className="topActions">
            <GlobalSearch />
            <PortalQuickAccess active={active} title={title} section={section?.label || null} />
            <NotificationBell />
            <span className="systemPill">
              <i />
        ...
        <div className="content">
          {section ? (
            <nav aria-label="Fil d’Ariane" className="breadcrumbBar">
              <div className="breadcrumbTrail">
                <Link href="/portal/sections/home">DONI</Link>
              ...
              {!isSectionHome ? (
                <Link className="btn" href={section.href}>
                  ← Retour à {section.label}
                </Link>
              ) : null}
            </nav>
          ) : null}
          {section && !isSectionHome && contextTools.length ? (
            <nav className="sectionContextBar" aria-label={`Actions rapides · ${section.label}`}>
              <span className="sectionContextLabel">Actions rapides</span>
              <div className="sectionContextLinks">
                {contextTools.map((tool) => {
                  const isCurrent = active === tool.href || active.startsWith(`${tool.href}/`);
                  return (
                    <Link
                      key={tool.href}
                      href={tool.href}
