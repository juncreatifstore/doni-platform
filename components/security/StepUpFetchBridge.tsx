...
async function securityRequirement(response: Response) {
  ...
  try {
    ...
    return body?.error === 'step_up_required'
      ? 'MFA'
      : body?.error === 'password_reauth_required'
        ? 'PASSWORD'
        : null;
  } catch {
    return null;
...
async function passwordReauth(original: typeof window.fetch) {
  ...
  const r = await original('/api/auth/reauth/password', {
    method: 'POST',
...
export function StepUpFetchBridge() {
  useEffect(() => {
    ...
    const wrapped: typeof window.fetch = async (input, init) => {
      ...
      if (requirement === 'MFA') {
        if (!stepUpInFlight)
          stepUpInFlight = performStepUp().finally(() => {
            stepUpInFlight = null;
          });
        await stepUpInFlight;
      } else {
        if (!passwordReauthInFlight)
          passwordReauthInFlight = passwordReauth(original).finally(() => {
            passwordReauthInFlight = null;
          });
