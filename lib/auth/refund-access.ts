  ...
  const rows: any[] = await (db as any).refundRequest.findMany({
    where: {
      OR: [
        ...
        { ticketReference: { in: tickets.map((t) => t.reference) } },
      ],
    },
    select: { id: true },
  });
...
export async function refundAllowed(user: SafeUser, refundId: string) {
  ...
  const row: any = await (db as any).refundRequest.findUnique({
    where: { id: refundId },
    select: { paymentId: true, ticketReference: true },
  });
  if (!row) return false;
  if (row.paymentId) {
    const count = await db.payment.count({
      where: { id: String(row.paymentId), conversation: { country: scope.country } },
    });
...
export async function refundInputAllowed(user: SafeUser, input: any) {
  ...
  let reference = String(input?.reference || '')
    .trim()
    .toUpperCase();
  if (!reference && postBookingId) {
    const pb: any = await (db as any).postBookingRequest.findUnique({
      where: { id: postBookingId },
      select: { reference: true },
    });
    reference = String(pb?.reference || '')
      .trim()
      .toUpperCase();
  }
