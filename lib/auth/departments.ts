...
export function departmentLabel(value: string | null | undefined) {
  return value && isDepartment(value) ? DEPARTMENT_LABELS[value] : 'Équipe générale';
}

function key(userId: string) {
  return `user.department.${userId}`;
}
...
export async function setUserDepartment(userId: string, department: Department | null, updatedBy?: string) {
  ...
  if (!department) {
    await db.appSetting.deleteMany({ where: { key: settingKey } }).catch(() => {});
    return;
  }
  await db.appSetting.upsert({
    where: { key: settingKey },
    create: { key: settingKey, category: 'Users', value: department, updatedBy },
    update: { value: department, updatedBy },
  });
}
export async function getDepartmentsForUsers(userIds: string[]) {
  ...
  for (const row of rows) {
    const id = row.key.replace('user.department.', '');
    if (isDepartment(row.value)) out.set(id, row.value);
  }
  return out;
}
