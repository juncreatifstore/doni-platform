...
export function filterManualPaymentRowsForUser<
  T extends { payment?: { conversation?: { country?: string | null } | null } | null },
>(rows: T[], user: SafeUser) {
...
export async function assertManualPaymentReviewAccess(id: string, user: SafeUser) {
  ...
  const row = await db.manualPaymentReview.findUnique({
    where: { id },
    select: { payment: { select: { conversation: { select: { country: true } } } } },
  });
