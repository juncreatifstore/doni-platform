...
export async function requireFactorChangeAuthorization(userId: string) {
  ...
  if (hasExistingFactor) {
    if (!(await hasRecentStepUp()))
      return { ok: false as const, status: 428, error: 'step_up_required', proof: 'MFA' as const };
    return { ok: true as const, proof: 'MFA' as const };
  }
  if (!(await hasRecentPasswordReauth()))
    return { ok: false as const, status: 428, error: 'password_reauth_required', proof: 'PASSWORD' as const };
