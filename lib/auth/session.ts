...
export type SafeUser = Pick<PortalUser,'id'|'username'|'fullName'|'email'|'role'|'country'|'active'> & {department:Department|null};
export function safeUser(u: PortalUser, department:Department|null=null): SafeUser { return { id:u.id,username:u.username,fullName:u.fullName,email:u.email,role:u.role,country:u.country,active:u.active,department }; }
...
export async function getCurrentUser(): Promise<SafeUser|null> {
  ...
  return safeUser(session.user,department);
}
