...
export async function generateCopilotDraft(recommendationId:string){
 ...
 const audience=r.type==='RECOVERY'?'Client individuel à forte intention':r.type==='LIVE_OFFER'?'Audience liée à la route et aux recherches récentes':r.type==='WINNING_PATTERN'?'Audience comparable à celle du test gagnant, à revalider avant utilisation':'Audience ayant recherché cette destination récemment';
 const objective=r.type==='RECOVERY'?'Récupérer une réservation abandonnée':r.type==='LIVE_OFFER'?'Transformer une offre live en opportunité marketing':r.type==='WINNING_PATTERN'?'Réutiliser un apprentissage A/B validé dans un nouveau test/campagne contrôlé':'Capitaliser sur la demande détectée';
 const message=r.type==='RECOVERY'?`Votre recherche ${r.route||''} est toujours intéressante. Nous pouvons vérifier le tarif et la disponibilité actuels avant de poursuivre votre réservation.`:r.type==='LIVE_OFFER'?`Une opportunité de voyage ${r.route||''} vient d’être détectée. Tarif et disponibilité doivent être revérifiés juste avant toute communication.`:r.type==='WINNING_PATTERN'?`Brouillon inspiré du pattern gagnant validé par l’expérience ${r.experimentId||''}. Adapter le contenu à l’audience actuelle, puis soumettre à validation humaine avant toute diffusion.`:`La demande vers ${r.destination||'cette destination'} augmente. Préparer une campagne ciblée uniquement après validation d’une offre réelle.`;
 const value={id,status:'AI_READY',recommendationId:r.id,type:r.type,priority:r.priority,score:r.score,title:r.title,objective,audience,message,channelSuggestion:r.type==='RECOVERY'?'WhatsApp 1:1':r.type==='WINNING_PATTERN'?'Canal du pattern source, à confirmer':'WhatsApp + social + email',budgetSuggestion:r.type==='DEMAND'?'À définir après validation d’une offre live':'Aucune dépense automatique',conversationId:r.conversationId||null,destination:r.destination||null,route:r.route||null,amount:r.amount||null,currency:r.currency||null,learningCampaignId:r.learningCampaignId||null,experimentId:r.experimentId||null,source:r.source,approvalRequired:true,publicationAllowed:false,disclaimer:'Brouillon préparé par DONI. Validation humaine obligatoire. Consentement, audience, disponibilité et toute mention de prix doivent être revérifiés avant envoi ou publication.',createdAt:now};
