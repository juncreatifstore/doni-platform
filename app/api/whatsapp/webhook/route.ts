...
import {
  ...
  recordMarketingReply,
} from '@/lib/workspace/marketing-delivery-tracking';
...
function extractStatuses(payload: any) {
  ...
  return out.filter((x) => x.providerMessageId && x.status);
}

async function dispatchWithRetry(waId: string, text: string, raw: unknown) {
  try {
    return await dispatchInbound(waId, text, raw);
  } catch (first) {
...
export async function GET(req: Request) {
  ...
  if (mode === 'subscribe' && token === (await getSetting<string>('whatsapp.verify_token')))
...
export async function POST(req: Request) {
  ...
  for (const s of statuses)
    statusResults.push(
      await recordMarketingDeliveryStatus(s).catch(() => ({ matched: false, error: true }) as const),
    );
  ...
  for (const m of messages) {
    ...
    try {
      ...
      await ensureCustomerProfile({
        ...
        language: session.lockedLanguage,
        country: session.country,
      }).catch(() => null);
      const raw: any = m.raw;
      const contentType = String(raw?.type || 'text');
      await recordMessage({
        conversationId: session.id,
        direction: 'INBOUND',
        senderType: 'CUSTOMER',
        ...
        metadata: { rawType: contentType },
      });
      await db.doniConversation.update({ where: { id: session.id }, data: { lastMessageAt: new Date() } });
      const preference =
        contentType === 'text'
          ? await handleMarketingPreferenceCommand(m.waId, m.text).catch(() => ({ handled: false }) as const)
          : ({ handled: false } as const);
      if (preference.handled) {
        const delivery = await sendWhatsAppText(m.waId, preference.reply);
        const providerMessageId = (delivery as any)?.response?.messages?.[0]?.id ?? null;
        await recordMessage({
          conversationId: session.id,
          direction: 'OUTBOUND',
          senderType: 'BOT',
          text: preference.reply,
          providerMessageId,
          metadata: { delivery, event: 'marketing_preference', status: preference.status },
        });
        processed.push({
          messageId: m.messageId,
          waId: m.waId,
          sessionId: session.id,
          marketingPreference: preference.status,
          delivery,
        });
        continue;
      }
      ...
      const fresh = await db.doniConversation
        .findUnique({ where: { id: result.sessionId } })
        .catch(() => null);
      const freshState = (fresh?.state || {}) as any;
      await ensureCustomerProfile({
        waId: m.waId,
        conversationId: result.sessionId,
        displayName: m.displayName,
        language: fresh?.lockedLanguage || fresh?.language || null,
        email: freshState.contact_email || freshState.email || null,
        country: fresh?.country || null,
      }).catch(() => null);
      const delivery = result.reply.message
        ? await sendWhatsAppText(m.waId, result.reply.message)
      ...
      if (result.reply.message) {
        ...
        await recordMessage({
          ...
          providerMessageId,
          metadata: { delivery, nextSegment: result.segment, outcome: result.reply.outcome },
        });
      }
      processed.push({
        messageId: m.messageId,
      ...
      const first = (error as any)?.__firstDispatchError ?? null;
      console.error('whatsapp_dispatch_failed', {
        waId: m.waId,
        messageId: m.messageId,
        error: code,
        firstError: first,
      ...
      if (sessionId) {
        await recordMessage({
          ...
          direction: 'OUTBOUND',
          senderType: 'SYSTEM',
          text: '⚠️ Message entrant reçu mais traitement DONI interrompu.',
          metadata: {
            event: 'whatsapp_dispatch_failed',
            providerMessageId: m.messageId,
            error: code,
            firstError: first,
          },
  ...
  return Response.json(
    {
      ...
      statusMatched: statusResults.filter((x: any) => x?.matched).length,
      processed,
    },
    { status: 200 },
  );
