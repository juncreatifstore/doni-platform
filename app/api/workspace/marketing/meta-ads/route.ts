...
import {
  ...
  saveMetaAdsSelection,
  setMetaAdsWriteEnabled,
} from '@/services/facebook/ads';
...
export async function GET() {
  ...
  try {
    const [status, accounts] = await Promise.all([getMetaAdsStatus(), listAccessibleMetaAdAccounts()]);
    return NextResponse.json({ success: true, status, accounts });
  } catch (e) {
    return NextResponse.json(
      { success: false, error: e instanceof Error ? e.message : 'meta_ads_diagnostic_failed' },
      { status: 500 },
    );
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (body.action === 'select_account') {
      await saveMetaAdsSelection(
        {
          adAccountId: String(body.adAccountId || ''),
          businessId: body.businessId == null ? undefined : String(body.businessId),
        },
        auth.user.id,
      );
    ...
    if (body.action === 'set_write_enabled') {
      ...
      if (enabled && body.confirmation !== 'ENABLE_META_ADS_WRITES')
        return NextResponse.json(
          { success: false, error: 'explicit_confirmation_required' },
          { status: 400 },
        );
    ...
    if (body.action === 'create_paused_campaign') {
      ...
      if (!campaignId)
        return NextResponse.json({ success: false, error: 'campaign_id_required' }, { status: 400 });
      if (body.confirmation !== 'CREATE_META_CAMPAIGN_PAUSED')
        return NextResponse.json(
          { success: false, error: 'explicit_confirmation_required' },
          { status: 400 },
        );
    ...
    return NextResponse.json(
      { success: false, error: e instanceof Error ? e.message : 'meta_ads_update_failed' },
      { status: 500 },
    );
  }
}
