...
export async function GET(){
 ...
 const [campaigns,users]=await Promise.all([listMarketingCampaigns(),marketingCampaignAssignees()]);
...
export async function POST(req:Request){
 ...
 try{
  ...
  const campaign=await createMarketingCampaign({...b,createdById:auth.user.id});
