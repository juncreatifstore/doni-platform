...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!body?.reference || !body?.pnr)
      return NextResponse.json({ success: false, error: 'reference_and_pnr_required' }, { status: 400 });
    ...
    await audit({
      ...
      entity: 'Ticket',
      entityId: t.id,
      metadata: { reference: t.reference, pnr: t.pnr, country: auth.user.country || null },
    });
    return NextResponse.json({ success: true, ticket: t });
  } catch (e) {
    return NextResponse.json(
      { success: false, error: e instanceof Error ? e.message : 'issue_failed' },
      { status: 400 },
    );
  }
}
