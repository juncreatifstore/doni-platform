...
export async function GET() {
  ...
  if (!canAccessDepartments(auth.user, ['FINANCE', 'MANAGEMENT']))
  ...
  try {
    return NextResponse.json({ success: true, data: await getFinanceMetrics(scope) });
  } catch (e) {
    return NextResponse.json(
      { success: false, error: e instanceof Error ? e.message : 'finance_failed' },
      { status: 500 },
    );
  }
}
