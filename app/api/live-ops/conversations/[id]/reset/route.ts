...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await db.doniConversation.findUnique({
    where: { id },
    select: { id: true, waId: true, currentSegment: true, status: true },
  });
  if (!existing)
  ...
  await audit({
    ...
    entityId: id,
    metadata: { waId: existing.waId, fromSegment: existing.currentSegment, reason: 'controlled_test' },
  });
  return NextResponse.json({
    ...
    conversation: {
      ...
      currentSegment: conversation.currentSegment,
