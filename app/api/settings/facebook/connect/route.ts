...
export async function GET(req: Request) {
  ...
  if (!step.ok) {
    const u = new URL('/step-up', req.url);
    u.searchParams.set('returnTo', '/api/settings/facebook/connect');
    return NextResponse.redirect(u);
  }
  const appId = await getSetting<string>('facebook.app_id');
  const version = await getSetting<string>('facebook.graph_version');
  if (!appId) return NextResponse.redirect(new URL('/settings?facebook_error=app_id_missing', req.url));
  const state = randomUUID();
  const expiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();
  await db.appSetting.create({
    data: {
      key: `facebook.oauth.state.${state}`,
      category: 'Facebook OAuth',
      value: { state, userId: auth.user.id, expiresAt, mfaVerified: true } as any,
    },
  });
