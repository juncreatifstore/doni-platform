...
export async function GET(req: Request) {
  ...
  if (denied)
    return NextResponse.redirect(
      new URL(`/marketing/campaigns?meta_ads_error=${encodeURIComponent(denied)}`, req.url),
    );
  const row = state
    ? await db.appSetting.findUnique({ where: { key: `facebook.ads.oauth.state.${state}` } })
    : null;
  ...
  if (!valid || !code)
    return NextResponse.redirect(new URL('/marketing/campaigns?meta_ads_error=oauth_state_invalid', req.url));
  ...
  if (!appId || !appSecret)
    return NextResponse.redirect(
      new URL('/marketing/campaigns?meta_ads_error=app_credentials_missing', req.url),
    );
  ...
  if (!shortRes.ok || !shortJson?.access_token)
    return NextResponse.redirect(
      new URL('/marketing/campaigns?meta_ads_error=token_exchange_failed', req.url),
    );
  ...
  if (longRes.ok && longJson?.access_token) token = String(longJson.access_token);
