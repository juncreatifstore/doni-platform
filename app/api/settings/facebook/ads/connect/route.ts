...
export async function GET(req: Request) {
  ...
  if (!step.ok) {
    ...
    u.searchParams.set('returnTo', '/api/settings/facebook/ads/connect');
    return NextResponse.redirect(u);
  }
  const appId = await getSetting<string>('facebook.app_id');
  const version = await getSetting<string>('facebook.graph_version');
  if (!appId)
    return NextResponse.redirect(new URL('/marketing/campaigns?meta_ads_error=app_id_missing', req.url));
  const state = randomUUID();
  const expiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();
  await db.appSetting.create({
    data: {
      key: `facebook.ads.oauth.state.${state}`,
      category: 'Meta Ads OAuth',
      value: { state, userId: auth.user.id, expiresAt, stepUpVerified: true } as any,
    },
  });
