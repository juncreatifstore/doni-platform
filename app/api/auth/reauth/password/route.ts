...
export async function POST(req: Request) {
  ...
  if (!password) return NextResponse.json({ success: false, error: 'password_required' }, { status: 400 });
  ...
  const verifiedAt = await markCurrentSessionPasswordReauth();
  return NextResponse.json({ success: true, verifiedAt, validForSeconds: 300 });
}
