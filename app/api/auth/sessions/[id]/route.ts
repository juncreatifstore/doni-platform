...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await revokeUserSession(auth.user.id, id);
    await audit({
      userId: auth.user.id,
    ...
    const error = e instanceof Error ? e.message : 'session_revoke_failed';
    const status =
      error === 'session_not_found' ? 404 : error === 'cannot_revoke_current_session_here' ? 400 : 400;
