...
export async function POST(req: Request) {
  ...
  try {
    const options = await registrationOptions(req, auth.user);
    await audit({
      userId: auth.user.id,
      action: 'AUTH_PASSKEY_ENROLLMENT_STARTED',
      entity: 'PortalUser',
    ...
    return NextResponse.json(
      { success: false, error: e instanceof Error ? e.message : 'passkey_options_failed' },
      { status: 400 },
    );
  }
}
