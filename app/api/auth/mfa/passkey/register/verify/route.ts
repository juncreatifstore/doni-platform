...
export async function POST(req: Request) {
  ...
  const gate = await requireFactorChangeAuthorization(auth.user.id);
  if (!gate.ok) return NextResponse.json({ success: false, error: gate.error }, { status: gate.status });
  try {
    const b = await req.json();
    if (!b?.response)
      return NextResponse.json({ success: false, error: 'response_required' }, { status: 400 });
    const credential = await verifyAndStoreRegistration(req, auth.user.id, b.response, b.name);
    await audit({
      userId: auth.user.id,
      action: 'AUTH_PASSKEY_ENROLLED',
      entity: 'PortalUser',
      entityId: auth.user.id,
      metadata: { credentialId: credential.id, proof: gate.proof },
    });
    ...
    return NextResponse.json(
      { success: false, error: e instanceof Error ? e.message : 'passkey_registration_failed' },
      { status: 400 },
    );
  }
}
