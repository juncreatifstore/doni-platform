...
export async function POST(req: Request) {
  ...
  try {
    ...
    const recoveryCodes = await confirmTotpEnrollment(auth.user.id, code);
    await audit({
      userId: auth.user.id,
      action: 'AUTH_MFA_TOTP_ENABLED',
      entity: 'PortalUser',
    ...
    return NextResponse.json(
      { success: false, error: e instanceof Error ? e.message : 'mfa_confirm_failed' },
      { status: 400 },
    );
  }
}
