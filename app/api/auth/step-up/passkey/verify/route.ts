...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (current.userId !== auth.user.id)
      return NextResponse.json({ success: false, error: 'forbidden' }, { status: 403 });
    try {
      const verified = await verifyAuthentication(req, challenge, auth.user.id, b.response);
      await completeMfaChallenge(challenge, auth.user.id);
      await markCurrentSessionMfaVerified('PASSKEY');
      await audit({
        userId: auth.user.id,
        action: 'AUTH_STEP_UP',
    ...
    const error = e instanceof Error ? e.message : 'passkey_step_up_failed';
    const status = [
