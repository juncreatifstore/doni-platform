...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (verified.userId !== auth.user.id)
      return NextResponse.json({ success: false, error: 'forbidden' }, { status: 403 });
    await markCurrentSessionMfaVerified(verified.method);
    await audit({
      userId: auth.user.id,
      action: 'AUTH_STEP_UP',
    ...
    return NextResponse.json({ success: true, method: verified.method });
  } catch (e) {
    const error = e instanceof Error ? e.message : 'step_up_failed';
