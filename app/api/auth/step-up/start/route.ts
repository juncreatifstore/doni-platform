...
export async function POST(req: Request) {
  ...
  const [totp, passkey] = await Promise.all([getTotpStatus(auth.user.id), getPasskeyStatus(auth.user.id)]);
  const privileged = auth.user.orgRole === 'SUPER_ADMIN' || auth.user.orgRole === 'COUNTRY_ADMIN';
  ...
  if (!methods.length)
    return NextResponse.json({ success: false, error: 'mfa_not_configured' }, { status: 409 });
  const challenge = await createMfaChallenge(auth.user.id, methods);
  return NextResponse.json({ success: true, challenge, methods });
}
