...
export async function POST(req: Request) {
  ...
  try {
    ...
    await audit({
      userId: auth.user.id,
      action: 'FLIGHT_DISRUPTION_TARGETED_DELIVERY_TEST',
      entity: 'Ticket',
      metadata: { reference, sent: result.sent, failed: result.failed, eligible: result.eligible },
    });
    return NextResponse.json({ success: true, ...result });
  } catch (e) {
    const error = e instanceof Error ? e.message : 'targeted_disruption_delivery_failed';
    const status =
      error === 'tracking_test_delivery_disabled' ||
      error === 'automatic_delivery_gates_disabled' ||
      error === 'standard_alerts_must_remain_disabled'
        ? 403
        : 400;
