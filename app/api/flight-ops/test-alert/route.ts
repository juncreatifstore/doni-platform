...
import {
  classifyFlightDisruption,
  disruptionMessage,
  type FlightDisruptionType,
} from '@/services/operations/incidents';
...
export async function POST(req: Request) {
  ...
  if (!(await getSetting<boolean>('tracking.test_alert_enabled')))
    return NextResponse.json({ success: false, error: 'tracking_test_alert_disabled' }, { status: 403 });
  if (!(await getSetting<boolean>('whatsapp.enabled')))
    return NextResponse.json({ success: false, error: 'whatsapp_disabled' }, { status: 403 });
  try {
    ...
    if (!['delay', 'cancelled', 'diverted', 'checkin_open'].includes(scenario))
      return NextResponse.json({ success: false, error: 'invalid_scenario' }, { status: 400 });
    const tracking: any = await (db as any).flightTracking.findFirst({
      where: { ticketReference: reference, active: true },
      orderBy: { scheduledDeparture: 'asc' },
    });
    if (!tracking) return NextResponse.json({ success: false, error: 'tracking_not_found' }, { status: 404 });
    if (!tracking.clientPhone)
      return NextResponse.json({ success: false, error: 'tracking_phone_missing' }, { status: 400 });
    ...
    await audit({
      userId: auth.user.id,
      action: 'FLIGHT_ALERT_TEST',
      entity: 'FlightTracking',
      entityId: tracking.id,
      metadata: { reference, scenario, flight: `${tracking.airlineCode}${tracking.flightNumber}` },
    });
    return NextResponse.json({
      success: true,
      scenario,
      reference,
      flight: `${tracking.airlineCode}${tracking.flightNumber}`,
      route: `${tracking.origin}-${tracking.destination}`,
      classifications,
      sent: true,
      mutatedFlightState: false,
    });
  } catch (e) {
    return NextResponse.json(
      { success: false, error: e instanceof Error ? e.message : 'test_alert_failed' },
      { status: 400 },
    );
  }
}
