    ...
    <DoniShell title="Flow Tracker" active="/flow-tracker" user={user}>
      ...
      <div className="grid">
        ...
        <MetricCard
          ...
          value={d.paid24}
          note={`${d.paymentConversionPct}% de conversion`}
          tone={d.paymentConversionPct >= 30 ? 'good' : 'warn'}
        />
        <MetricCard
          label="Tickets émis · 24 h"
          value={d.issued24}
          note={`${d.issueConversionPct}% des conversations`}
        />
        <MetricCard
          label="Flux critiques"
          value={d.criticalCount}
          note="Bloqués > 30 min ou agent requis"
          tone={d.criticalCount ? 'bad' : 'good'}
        />
      </div>
