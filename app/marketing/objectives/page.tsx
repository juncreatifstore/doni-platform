...
function responseValue(v:number|null){return v===null?'N/A':`${v.toFixed(v%1?1:0)} min`}
...
export default async function Page(){
 ...
 return <DoniShell title="Objectifs marketing" active="/marketing/objectives" user={user}>
  ...
  <div className="marketingScorecardGrid">
   <section className="card marketingGoalPanel"><h3>Réactivité WhatsApp · {d.response.periodDays} jours</h3><div className="marketingManualGoal"><span>Première réponse DONI</span><strong>{responseValue(d.response.firstDoni.averageMinutes)}</strong><small>{d.response.firstDoni.withinTargetPct===null?'Pas assez de données':`${d.response.firstDoni.withinTargetPct}% des séquences sous ${d.response.targetMinutes} min · ${d.response.firstDoni.samples} échantillon(s)`}</small></div><div className="marketingManualGoal"><span>Réponse humaine</span><strong>{responseValue(d.response.human.averageMinutes)}</strong><small>{d.response.human.withinTargetPct===null?'Aucun échantillon humain exploitable':`${d.response.human.withinTargetPct}% sous ${d.response.targetMinutes} min · ${d.response.human.samples} échantillon(s)`}</small></div></section>
