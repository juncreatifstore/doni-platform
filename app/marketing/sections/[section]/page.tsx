...
export default async function Page({ params }: { params: Promise<{ section: string }> }) {
  ...
  const section = getMarketingSection(key);
  if (!section) notFound();
  const tools = section.tools.filter((tool) => !tool.minimum || hasRole(user.role, tool.minimum));
  return (
    <DoniShell title={`Marketing · ${section.label}`} active={section.href} user={user}>
      <section className="workspaceWelcome">
        <div>
          <span className="workspaceKicker">Marketing · Navigation simplifiée</span>
          <h2>
        ...
        <div>
          <Link className="btn" href="/marketing/sections/overview">
            ← Vue d’ensemble
          </Link>
        </div>
      </section>
      <section className="card">
        <div className="searchIntelHead">
          <div>
            ...
            <h3>Choisir un espace de travail</h3>
          </div>
          <small>Chaque outil conserve son écran et son URL existants</small>
        </div>
        <div
          style={{
            display: 'grid',
          ...
          {tools.map((tool) => (
            <Link key={tool.href} href={tool.href} style={{ textDecoration: 'none', color: 'inherit' }}>
              ...
              >
                ...
                <span style={{ marginTop: 6, fontWeight: 700 }}>Ouvrir →</span>
              </article>
            </Link>
          ))}
        </div>
