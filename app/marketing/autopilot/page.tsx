...
function mexicoCityIso(value:string){
 // datetime-local contains no timezone. DONI marketing policy is America/Mexico_City (UTC-06:00).
 // Attach the policy offset before Date parsing so Vercel/Node UTC does not treat local clock time as UTC.
...
export default async function Page({searchParams}:{searchParams?:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <DoniShell title="Marketing Autopilot contrôlé" active="/marketing/autopilot" user={user}>
  ...
  <section className="card autopilotQueue"><div className="searchIntelHead"><div><span>File de contrôle</span><h3>Brouillons et campagnes</h3></div><small>{admin?'Mode approbateur':'Mode préparateur'}</small></div>
   <div className="autopilotGrid">{data.items.length?data.items.map(x=><article className="autopilotRow" key={x.id}><div><span className={`autopilotStatus ${x.status.toLowerCase()}`}>{label(x.status)}</span><h4>{x.title}</h4><small>{x.objective}</small></div><div><span>Audience</span><strong>{x.audience||'—'}</strong><small>{x.channelSuggestion||'Canal à définir'}</small></div><div><span>Contrôle</span><strong>{x.approvedBy?`Approuvé par ${x.approvedBy}`:'Approbation requise'}</strong><small>{x.scheduledFor?`Planifié : ${new Date(x.scheduledFor).toLocaleString('fr-FR',{timeZone:'America/Mexico_City'})}`:'Non planifié'}</small></div><div className="autopilotActions">
