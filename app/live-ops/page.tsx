...
export default async function Page() {
  ...
  if (!canAccessDepartments(user, ['CUSTOMER_SERVICE', 'OPERATIONS', 'MANAGEMENT', 'RESERVATIONS']))
    return (
      <DoniShell title="Live Ops" active="/live-ops" user={user}>
        <div className="card">
          <strong>Accès Live Ops non autorisé pour ce département.</strong>
        </div>
      </DoniShell>
    );
  const rows = await listLiveConversations(100, user);
  return (
    <DoniShell title="Live Ops" active="/live-ops" user={user}>
      <LiveOpsSummary />
      <h2 className="sectionTitle">Centre de conversations</h2>
