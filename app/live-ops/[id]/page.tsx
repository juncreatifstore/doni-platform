...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const row = await getConversationDetail(id);
  if (!row) notFound();
  return (
    <DoniShell title="Conversation" active="/live-ops" user={user}>
      <ConversationViewer initial={JSON.parse(JSON.stringify(row))} user={user} />
    </DoniShell>
  );
}
