...
export default async function Page({ searchParams }: { searchParams: Promise<{ reference?: string }> }) {
  ...
  const filtered = reference
    ? raw.filter((r) => String(r.tracking?.ticketReference || '').toUpperCase() === reference)
    : raw;
  ...
  return (
    <DoniShell title="Check-in Center" active="/checkin" user={user}>
      {reference ? (
        <div className="card" style={{ marginBottom: 16 }}>
          <strong>Réservation {reference}</strong>
          {!reservation ? (
            <p className="muted" style={{ marginBottom: 0 }}>
              Réservation introuvable ou hors de ton périmètre.
            </p>
          ) : reservation.checkinEligible ? (
            <p style={{ marginBottom: 0 }}>
              ✅ Fenêtre check-in disponible. Départ dans environ{' '}
              <strong>{reservation.hoursToDeparture} h</strong>. Les services ci-dessous restent traités par
              un agent.
            </p>
          ) : (
            <p className="muted" style={{ marginBottom: 0 }}>
              ⏳ Check-in pas encore disponible pour cette réservation. Départ dans environ{' '}
              {reservation.hoursToDeparture ?? '—'} h.
            </p>
          )}
        </div>
      ) : null}
