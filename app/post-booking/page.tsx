...
export default async function PostBookingPage() {
  ...
  if (!canAccessDepartments(user, ['CUSTOMER_SERVICE', 'TICKETING', 'OPERATIONS', 'MANAGEMENT']))
    ...
    <DoniShell title="Post-Booking" active="/post-booking" user={user}>
      ...
      <div className="grid">
        <div className="metric card">
          <span className="metricLabel">Demandes ouvertes</span>
          <strong className="metricValue">{open}</strong>
          <small className="muted">{rows.length} au total</small>
        </div>
        <div className="metric card">
          <span className="metricLabel">Attente compagnie</span>
          <strong className="metricValue">{waitingAirline}</strong>
          <small className="muted">Réponse fournisseur requise</small>
        </div>
        <div className="metric card">
          <span className="metricLabel">Paiements client</span>
          <strong className="metricValue">{waitingPayment}</strong>
          <small className="muted">Frais/penalités à encaisser</small>
        </div>
      </div>
            ...
            {rows.length ? (
              rows.map((r: any) => (
                <tr key={r.id}>
                  ...
                  <td>
                    <span className={statusBadge(r.status)}>{labels[r.status] || r.status}</span>
                  </td>
                  <td>{r.priority || '—'}</td>
                  <td>{r.phone || '—'}</td>
                  <td>
                    {r.penaltyAmount != null
                      ? `${Number(r.penaltyAmount).toFixed(2)} ${r.penaltyCurrency || 'USD'}`
                      : '—'}
                  </td>
              ...
              <tr>
                <td colSpan={7} className="muted">
                  Aucune demande post-booking pour le moment.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        ...
        <p className="muted" style={{ marginBottom: 0 }}>
          Annulation, correction de nom, changement de vol, changement de méthode de paiement, suivi des
          pénalités, confirmation compagnie et traitement des remboursements sont déjà pris en charge par le
          backend DONI.
        </p>
      </div>
    </DoniShell>
