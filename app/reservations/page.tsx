...
export default async function ReservationsPage(){
  ...
  return <DoniShell title="Mes réservations" active="/reservations" user={user}>
    <div className="card" style={{marginBottom:16}}>
      ...
      <p className="muted" style={{marginBottom:0}}>Consultez les billets émis, leurs PNR par segment, le suivi de vol et les actions disponibles. Le renvoi d’un e-ticket respecte le garde-fou de livraison.</p>
    </div>
    ...
    {!rows.length?<div className="card"><span className="muted">Aucune réservation émise.</span></div>:rows.map((r:any)=><div className="card" key={r.reference} style={{marginBottom:16}}>
      ...
      <h3 className="sectionTitle" style={{marginTop:22}}>Itinéraire</h3>
      <div className="tableWrap"><table className="table"><thead><tr><th>Vol</th><th>Trajet</th><th>Départ</th><th>Arrivée</th><th>PNR segment</th><th>Statut suivi</th></tr></thead><tbody>{(r.segments||[]).map((s:any,i:number)=>{const tr=(r.tracking||[])[i];return <tr key={i}><td>{`${s.airline||''} ${s.flightNumber||''}`.trim()||'—'}</td><td>{s.origin||'—'} → {s.destination||'—'}</td><td>{fmtDate(s.departureAt)}</td><td>{fmtDate(s.arrivalAt)}</td><td><strong>{s.pnr||s.bookingReference||r.pnr||'—'}</strong></td><td>{tr?.flightStatus||'scheduled'}</td></tr>})}</tbody></table></div>

      <div style={{marginTop:18,paddingTop:16,borderTop:'1px solid #e2e8f0'}}>
        <ReservationActions reference={r.reference} checkinEligible={Boolean(r.checkinEligible)} trackingReady={Boolean(r.trackingReady)}/>
        <div className="muted" style={{fontSize:12,marginTop:10}}>{r.checkinEligible?`Check-in disponible · départ dans environ ${r.hoursToDeparture} h`:'Check-in non disponible pour le moment'} · Alertes auto: {r.alertsEnabled?'ON':'OFF'} · Alertes disruption: {r.disruptionAlertsEnabled?'ON':'OFF'}</div>
      </div>
    </div>)}
  </DoniShell>;
}
